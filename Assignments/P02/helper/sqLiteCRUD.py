...
class SQLiteconn:
    ...
    def create_table(self, table_name, columns,unique_constraint):
        ...
        Args:
            ...
            unique_constraint : to handle duplicate insertion
        """
        try:
    ...
    def get_id(self, table_name, select_column, conditions):
        ''' This method will  get data from table

            - **Params:**
            - kwargs:  table_name ,select_colum, conditions
            
            - **Returns:**
            - (object) :list of selected columns
            '''
        try:
    ...
    def get_data(self, table_name, user_table,condition_id=None):
        """Read data from a table, optionally filtered by id with join with user_table
        
        Args:
            table_name (str): Name of the table.
            user_table: name of user table
            condition_id (int, optional): ID to use in the WHERE clause. Default is None.
        """
        try:
            # Construct the SQL query with an optional WHERE clause
    ...
    def calculate_folder_size(self,folder_id):
            ''' This method will calculate the recursively folder size
